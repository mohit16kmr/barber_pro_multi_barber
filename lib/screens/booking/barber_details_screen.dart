...
class BarberDetailsScreen extends StatefulWidget {
  ...
  const BarberDetailsScreen({super.key, required this.barberId});
...
class _BarberDetailsScreenState extends State<BarberDetailsScreen> {
  ...
  Future<void> _loadBarberDetails() {
    ...
    return barberProvider
        .getBarberById(widget.barberId)
        .then((barber) {
          if (!mounted) return;
          setState(() {
            if (barber == null) {
  ...
  void _createSampleBarber() {
    _barber = Barber(
      ...
      services: const [
        Service(name: 'Basic Haircut', price: 25.0, durationMinutes: 30),
        Service(name: 'Haircut + Fade', price: 35.0, durationMinutes: 45),
        Service(name: 'Full Beard Trim', price: 20.0, durationMinutes: 20),
        Service(name: 'Haircut + Beard', price: 45.0, durationMinutes: 60),
      ],
      photos: const [],
      queue: const [],
      currentToken: 5,
  ...
  Future<void> _bookNow(List<Service> selectedServices) async {
    ...
    // Show loading
    // Capture navigator & messenger before async gaps to avoid context usage
    ...
    try {
      ...
      // Ensure dialog closed and show error using captured messenger
      try {
        navigator.pop();
      } catch (_) {}
      messenger.showSnackBar(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    // Fire-and-forget load; navigate immediately so we don't use
                    // the captured BuildContext after an async gap.
                    final uid = auth.currentUser?.uid;
                    if (uid != null) bookingProvider.loadCustomerBookings(uid);
                    // Use flavor-safe navigation to avoid opening customer routes in barber flavor
                    flavorSafeGo(parentContext, '/bookings');
                  },
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  onPressed: () {
                    ...
                    if (!mounted) return;
                    // Use flavor-safe navigation to avoid opening customer routes in barber flavor
                    flavorSafeGo(parentContext, '/home');
                  },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Barber Details'), elevation: 0),
      body: _loading
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            await authProvider.toggleFavoriteBarber(
                              _barber!.barberId,
                            );
                          },
                        ),
